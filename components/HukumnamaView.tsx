  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-saffron-100">
      ...
      <div className="p-4 bg-gray-50 flex justify-center gap-2 border-b border-gray-200">
        <button 
          onClick={() => setLanguage('gurmukhi')}
          className={`px-3 py-1 rounded-full text-sm font-medium ${language === 'gurmukhi' ? 'bg-saffron-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-100'}`}
        >
          Gurmukhi
        </button>
        <button 
          onClick={() => setLanguage('punjabi')}
          className={`px-3 py-1 rounded-full text-sm font-medium ${language === 'punjabi' ? 'bg-saffron-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-100'}`}
        >
          Punjabi
        </button>
        <button 
          onClick={() => setLanguage('english')}
          className={`px-3 py-1 rounded-full text-sm font-medium ${language === 'english' ? 'bg-saffron-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-100'}`}
        >
          English
        </button>
      </div>

      <div className="p-8 text-center min-h-[400px] flex items-center justify-center bg-[url('https://www.transparenttextures.com/patterns/cream-paper.png')]">
        {language === 'gurmukhi' && (
